...
from core.train import train_src_adda, train_tgt_adda, train_src_robust
...
def main():
    ...
    # Load dataset
    svhn_dataloader = get_svhn(split='train', download=False)

    src_data_loader = get_mnist(train=True, download=False)
    src_data_loader_eval = get_mnist(train=False, download=False)
    tgt_data_loader = get_usps(train=True, download=False)
    tgt_data_loader_eval = get_usps(train=False, download=False)
    # Model init ADDA
    ...
    print(clf)
    src_encoder, clf = eval_src_robust(src_encoder, clf, src_data_loader)

    if not (src_encoder.pretrained and clf.pretrained and
            params.src_model_trained):
        src_encoder, clf = train_src_robust(src_encoder, clf, src_data_loader)
